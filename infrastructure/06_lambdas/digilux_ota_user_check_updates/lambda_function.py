...
def _handle_package_status(event: dict, user_id: str, path_params: dict,
    ...
    _log("info", "package_status_request",
         ...
         sourceIp=source_ip, requestId=request_id, table=PACKAGES_TABLE)

    if not pkg_name or not version:
        _log("warning", "package_status_missing_params",
             userId=user_id, packageName=pkg_name, version=version,
             requestId=request_id,
             detail="packageName and/or version blank or missing from path parameters")
        _audit("PACKAGE_STATUS_BAD_REQUEST", user_id,
               {"packageName": pkg_name, "version": version},
               "FAILURE",
               reason="missing_params", requestId=request_id)
        return _resp(400, {"error": "packageName and version are required"})

    _log("debug", "package_status_dynamo_start",
         userId=user_id, packageName=pkg_name, version=version,
         table=PACKAGES_TABLE, operation="get_item")

    t_dynamo = time.monotonic()
    try:
        item = dynamo.Table(PACKAGES_TABLE).get_item(
            Key={"packageName": pkg_name, "version": version}
        ).get("Item")
    except ClientError as e:
        dynamo_ms = int((time.monotonic() - t_dynamo) * 1000)
        _log("error", "package_status_dynamo_error",
             userId=user_id, packageName=pkg_name, version=version,
             awsError=e.response["Error"]["Code"],
             awsMessage=e.response["Error"]["Message"],
             table=PACKAGES_TABLE, dynamoMs=dynamo_ms, requestId=request_id,
             detail="DynamoDB get_item failed — cannot determine package status")
        _audit("PACKAGE_STATUS_ERROR", user_id,
               {"packageName": pkg_name, "version": version},
               "FAILURE",
               awsError=e.response["Error"]["Code"],
               requestId=request_id)
    ...
    dynamo_ms = int((time.monotonic() - t_dynamo) * 1000)
    _log("debug", "package_status_dynamo_complete",
         userId=user_id, packageName=pkg_name, version=version,
         itemFound=bool(item), dynamoMs=dynamo_ms)

    if not item:
        fn_ms = int((time.monotonic() - fn_start) * 1000)
        _log("warning", "package_status_not_found",
             userId=user_id, packageName=pkg_name, version=version,
             dynamoMs=dynamo_ms, fnMs=fn_ms, requestId=request_id,
             detail="No DynamoDB record for this packageName+version — "
                    "may have been deleted or never existed; treating as unsafe")
        _audit("PACKAGE_STATUS_NOT_FOUND", user_id,
               {"packageName": pkg_name, "version": version},
               "FAILURE",
               requestId=request_id,
               detail="Package record not found — device must not install")
        return _resp(404, {"error": "Package not found"})

    # ── Validate presence of critical fields ───────────────────────────────────
    if item.get("status") is None:
        _log("warning", "package_status_field_missing",
             userId=user_id, packageName=pkg_name, version=version,
             missingField="status",
             detail="'status' field absent from DynamoDB record — defaulting to UNKNOWN")
    if item.get("activated") is None:
        _log("warning", "package_status_field_missing",
             userId=user_id, packageName=pkg_name, version=version,
             missingField="activated",
             detail="'activated' field absent from DynamoDB record — defaulting to False")

    status       = item.get("status")    or "UNKNOWN"
    activated    = bool(item.get("activated", False))
    safe         = (status == "ACTIVE" and activated)
    release_type = item.get("releaseType",  "UNKNOWN")
    uploaded_by  = item.get("uploadedBy",   "unknown")
    processed_at = item.get("processedAt")
    fn_ms        = int((time.monotonic() - fn_start) * 1000)

    _log("debug", "package_status_item_detail",
         userId=user_id, packageName=pkg_name, version=version,
         status=status, activated=activated, releaseType=release_type,
         uploadedBy=uploaded_by, processedAt=processed_at,
         sha256Present=bool(item.get("sha256")),
         encKeyPresent=bool(item.get("encS3Key")),
         dynamoMs=dynamo_ms)

    if not safe:
        _log("warning", "package_status_unsafe",
             userId=user_id, packageName=pkg_name, version=version,
             status=status, activated=activated, releaseType=release_type,
             reason="status_not_active" if status != "ACTIVE" else "not_activated",
             detail="Device must not install — package is not in a safe state")

    _log("info", "package_status_result",
         userId=user_id, packageName=pkg_name, version=version,
         status=status, activated=activated, releaseType=release_type,
         safeToInstall=safe, dynamoMs=dynamo_ms, fnMs=fn_ms, requestId=request_id)

    _audit("PACKAGE_STATUS_CHECKED", user_id,
           {"packageName": pkg_name, "version": version},
           "SUCCESS" if safe else "BLOCKED",
           status=status, activated=activated, releaseType=release_type,
           safeToInstall=safe, uploadedBy=uploaded_by,
           dynamoMs=dynamo_ms, fnMs=fn_ms, requestId=request_id)
    ...
    try:
        ...
        if not user_id:
            _log("warning", "missing_sub_claim",
                 requestId=request_id,
                 path=event.get("path"), httpMethod=event.get("httpMethod"),
        ...
        if path_params.get("packageName") and path_params.get("version"):
            _log("debug", "routing_to_package_status",
                 userId=user_id, requestId=request_id,
                 packageName=path_params.get("packageName"),
                 version=path_params.get("version"),
                 httpMethod=event.get("httpMethod"),
                 path=event.get("path"))
            return _handle_package_status(event, user_id, path_params, request_id)
